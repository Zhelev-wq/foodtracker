...
function AddFoodForm(
    {FoodOutData, setVisible}: AddFoodFormProps
) {
    function ProcessData(FoodOutData: FoodOut){
        ...
        const details = Object.entries(FoodOutData).map(
            (pair) => 
                <li>{pair[0]}: {(pair[1] * ratio).toFixed(1) || 0}</li>
        )
        return details;
    }
    const {name, vitamins, minerals, fats, id, barcode, ...macros} = FoodOutData
...
function EditFoodForm(
    {FoodOutData, existingGrams, FoodEntryItemId, setVisible}: EditFoodProps
) {
...
export default function FoodEntryForm(
    ...
    if (mode == "edit") {
        ...
        return <EditFoodForm 
            ...
            existingGrams={existingGrams} FoodEntryItemId={FoodEntryItemId}/>
    } 
